---
title: "What time did Halloween the Game come out? Early Access & launch times (PT)"
description: "Halloween: The Game released Tuesday, September 8, 2026 at 9:00 AM PT. Digital Deluxe Early Access began September 4, 2026 at 9:00 AM PT."
category: guides
slug: early-access-release-time
status: needs-verification
featured: true
role: core
assetType: article
sidebar:
  order: 1
intents:
  - early-access-release-time
quickAnswer: "Halloween: The Game is out now. Full release was Tuesday, September 8, 2026 at 9:00 AM PT; Digital Deluxe Early Access began Friday, September 4, 2026 at 9:00 AM PT as historical reference."
relations:
  -
    slug: preload-download-status
    type: next-step
  -
    slug: standard-vs-deluxe-upgrade
    type: next-step
  -
    slug: multiplayer
    type: next-step
  -
    slug: pc/system-requirements
    type: next-step
cover: ../../../assets/covers/gameplay/extended-first-look.jpg
coverMedia:
  alt: "Haddonfield street at night in Halloween: The Game"
  kind: cover
  aspectRatio: 16:9
  sourceLabel: Official
changeSummary: "GSC INTENT — what-time primaryQuery; Tuesday Sep 8 weekday fix; who can play now."
eyebrow: Release Time
facts:
  -
    label: Early Access start
    value: "Friday, September 4, 2026 — 9:00 AM PT (historical Digital Deluxe window)"
  -
    label: Full launch
    value: "Tuesday, September 8, 2026 — 9:00 AM PT"
  -
    label: Release state
    value: "Full release complete; Standard Edition unlocked Sep 8 at 9:00 AM PT"
  -
    label: Preload
    value: "No universal publisher preload schedule — check your storefront"
---
<!--
  This file is generated from site-spec.yaml.
  Do not edit directly.
  Run npm run site:generate instead.
  Source: site-input/pages/early-access-release-time.md
-->
# What time did Halloween the Game come out? Early Access & launch times (PT)

Quick answer

- Halloween: The Game is out now — official full release was Tuesday, September 8, 2026 at 9:00 AM Pacific Time (PT).
- Digital Deluxe Early Access began Friday, September 4, 2026 at 9:00 AM PT (historical).
- Standard Edition unlocked at the full release time above.
- Preload availability and file sizes vary by storefront — see the [preload download status guide](/preload-download-status/) for platform-specific notes.

Official unlock times + compact timezone conversions

- Publisher authoritative unlock time to convert from: 9:00 AM Pacific Time (PT).
  - Early Access: Friday, September 4, 2026 — 9:00 AM PT.
  - Full launch: Tuesday, September 8, 2026 — 9:00 AM PT.

Conversions (informational)

| Region | Early Access (Sep 4 → Sep 5) | Full launch (Sep 8) |
| --- | ---: | ---: |
| Los Angeles / PT | Sep 4 — 9:00 AM | Sep 8 — 9:00 AM |
| New York / EDT | Sep 4 — 12:00 PM | Sep 8 — 12:00 PM |
| UTC | Sep 4 — 4:00 PM | Sep 8 — 4:00 PM |
| UK / BST | Sep 4 — 5:00 PM | Sep 8 — 5:00 PM |
| Central Europe / CEST | Sep 4 — 6:00 PM | Sep 8 — 6:00 PM |
| China / Singapore | Sep 5 — 12:00 AM | Sep 8 — 12:00 AM |
| Japan | Sep 5 — 1:00 AM | Sep 8 — 1:00 AM |
| Sydney / AEST | Sep 5 — 2:00 AM | Sep 8 — 2:00 AM |

Who had Early Access (historical)

1. Digital Deluxe purchasers were granted a four-day Early Access window that began on Sep 4, 2026 at 9:00 AM PT.
2. Purchases made during that window conferred the same Early Access entitlements for that period (historical).
3. Standard Edition players gained access at the full launch on Sep 8, 2026 at 9:00 AM PT.
4. For edition differences and upgrade options, see the [edition comparison](/standard-vs-deluxe-upgrade/).

How early is it?

- Digital Deluxe Early Access begins Sep 4, 2026 at 9:00 AM PT; the full public release opens Sep 8, 2026 at 9:00 AM PT — a four-day Advance Access window for Digital Deluxe purchasers.

If you did not gain access at launch

Follow these steps in order (conservative, storefront-level checks):

1. Confirm you converted 9:00 AM PT to your local timezone correctly (use the table above).
2. Restart the storefront or game client (Steam, Epic, PlayStation, Xbox) and sign back in to refresh entitlements.
3. Verify the purchase or entitlement on the storefront account that made the purchase.
4. Check the client’s download queue — allow any downloads or automatic updates to finish.
5. If the game still does not unlock after the above, consult the publisher’s official channels or the official preorder FAQ for any notices or troubleshooting steps.

Common mistakes to avoid

- Assuming Standard Edition unlocks early — it does not include the four-day Advance Access.
- Mixing up timezones — convert from the official 9:00 AM PT start.
- Checking the wrong storefront account for purchase entitlements.

Preload note

- The publisher has not published a single universal preload schedule. Preload availability and file sizes vary by storefront and platform. Check the [preload download status guide](/preload-download-status/) for the latest platform-specific preload notes and links.

Full release date

- Official full release: Tuesday, September 8, 2026 at 9:00 AM Pacific Time (PT).

Sources

- [Official launch announcement — Halloween: The Game is out now](https://halloweengame.com/news/halloween-the-game-out-now/)
- [Halloween: The Game on Steam](https://store.steampowered.com/app/3219630/Halloween/)
