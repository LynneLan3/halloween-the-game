...
intents:
  - story-challenges
  - chapter-challenges
quickAnswer: "The Prologue and each numbered story chapter list six challenge objectives in the current reference set. Chapter 2's sixth no-alert objective is supported by UI/live evidence, but challenge registration has version-sensitive caveats and no workaround is guaranteed."
relations:
  -
    slug: single-player
    type: related
...
changeSummary: "Chapter 2 Challenge 6 child owner linked; hub remains objective/reference list."
eyebrow: Story Challenges
facts:
  -
    label: Coverage
    value: "Prologue plus Chapters 1–5"
  -
    label: Current reference set
    value: Six objective wordings per chapter
  -
    label: Tracking
    value: "Version-sensitive; verify registration in the in-game UI"
---
<!--
...
6. Kill or drive away every Orderly before escaping
...
5. Kill any target with a thrown cleaver
6. Kill any three targets in Chapter 1
...
6. Chapter 2 Challenge 6: Complete the chapter without alerting anyone — see /challenges/chapter-2-challenge-6/

Chapter 3 — Returning Home
1. Execute Jennifer with the Campfire
2. Execute Thomas with the Weight Bench
3. Execute Marcus with the Motorcycle
4. Kill Gloria and Keith
5. Kill three targets with a Wood Axe
...
3. Kill Lynda with telephone while wearing the Sheet
4. Kill a Resident with a Bloodthirsty execution
5. Fully stalk Bob and Lynda while they make out in the living room
6. Complete the chapter matching the film's plot
...
4. Fully Stalk any police in the Wallace House
...
For the no-alert Chapter 2 Challenge 6 attempt and Patch 1.0.1 tracking status, use /challenges/chapter-2-challenge-6/. That child guide covers the best-supported Surprise/no-alert attempt, unproven anecdotes, and what to do when a clean run still fails to register; this page remains the objective/reference hub.
...
3. For execution- or environment-based objectives, review /michael-myers/abilities/ for execution types and context behavior, and consult /single-player/ for single-player session tips.
4. If you need additional confirmation for whether a run counted, cross-check any available in-game result screens and, where applicable, logs or notes such as /loomis-logs/.
...
Verification tips
- After completing an objective, open the challenge UI immediately (end-of-run screens may also show progress).
- If an objective didn’t register, retry the objective in a fresh run and note whether registration behavior changes.
